...
TGAImage::TGAImage(const int w, const int h, const int bpp, std::vector<uint8_t> new_bgra) : w(w), h(h), bpp(bpp), data(w* h* bpp, 0)
{
    new_bgra.clear();
    std::copy(new_bgra.begin(), new_bgra.end(), new_bgra.end());
}
...
bool TGAImage::read_tga_file1(const std::string filename) {
    std::ifstream in;
    in.open(filename, std::ios::binary);
    if (!in.is_open()) {
        std::cerr << "can't open file " << filename << "\n";
    ...
    size_t nbytes = bpp * w * h;    // 3 * 1024 * 1024
    data = std::vector<std::uint8_t>(nbytes, 0);
    if (3 == header.datatypecode || 2 == header.datatypecode) {
        in.read(reinterpret_cast<char*>(data.data()), nbytes);
        if (!in.good()) {
...
bool TGAImage::load_rle_data1(std::ifstream& in) {
    size_t pixelcount = w * h;
    size_t currentpixel = 0;
    size_t currentbyte = 0;
    TGAColor colorbuffer;
    do {
        ...
        if (chunkheader < 128) {
            ...
            for (int i = 0; i < chunkheader; i++) {
                std::uint8_t new_bgra[4] = { 0,0,255,0 };
                for (int t = 0; t < bpp; t++)
                {
                    data[currentbyte++] = new_bgra[t];
                }
